...
const page = () => {
  ...
  useEffect(() => {
    const fetchllAStudents = async () => {
      try {
        ...
        console.log("🚀 ~ All Sudents ~ data:", data)
        setAllStudents(data);
        setLoading(false);
      } catch (err) {
  ...
  const downloadExcel = (jsonData) => {
    const cleanedData = jsonData.map(({ name, rollNo, section, class: studentClass, contact, fatherName, motherName, house: houseColor, photoLink, address }) => ({
      Name: name,
      "Roll No": rollNo,
      Section: section,
      Class: studentClass,
